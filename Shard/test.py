...
while True:
    ...
    for evenement in pygame.event.get():
        if evenement.type == pygame.QUIT:
            pygame.quit()
            sys.exit()
        elif evenement.type == pygame.MOUSEBUTTONDOWN and not check_victory("X") and not check_victory("O"):
            ...
            if grille[ligne][colonne] is None:
                # Attribution du symbole au joueur actuel
                grille[ligne][colonne] = "X"
                joueur = 2

                if check_victory("X"):

                    print("{LIGHTRED_EX}Victoire de l'{GREEN}Algo {RESET_ALL}".format(LIGHTRED_EX=Fore.LIGHTRED_EX,
                                                                                      GREEN=Fore.GREEN,
                                                                                      RESET_ALL=Style.RESET_ALL))
                    print(
                        "{YELLOW}L'AI a mis un certain nombre independant de COUP: {CYAN} {AICOUP}/{DLCOUP}{YELLOW}({CYAN} {PERCENT}%{YELLOW}) {RESET_ALL}".format(
                            AICOUP=AICOUP, DLCOUP=DLCOUP, CYAN=Fore.CYAN, YELLOW=Fore.YELLOW,
                            PERCENT=(AICOUP / DLCOUP) * 100, LIGHTRED_EX=Fore.LIGHTRED_EX, GREEN=Fore.GREEN,
                            RESET_ALL=Style.RESET_ALL))
                    AICOUP = 0
                    DLCOUP = 0
                    WIN[1] = WIN[1] + 1
                    ...
                    print(
                        "{YELLOW} Elle a aussi gagner {LIGHTGREEN_EX} {WIN}{YELLOW} partie(s) sur {LIGHTRED_EX}{total}{YELLOW}({CYAN}{PERCENT}{YELLOW}%) \nEt sur toute c'est partie {CYAN}{PERCENT2}% {YELLOW}sont mis par l'IA{RESET_ALL}".format(
                            LIGHTGREEN_EX=Fore.LIGHTGREEN_EX, PERCENT2=(SAICOUP / SDLCOUP) * 100,
                            PERCENT=(WIN[0] / total) * 100, total=WIN[1], WIN=WIN[0], AICOUP=AICOUP, DLCOUP=DLCOUP,
                            CYAN=Fore.CYAN, YELLOW=Fore.YELLOW, LIGHTRED_EX=Fore.LIGHTRED_EX, GREEN=Fore.GREEN,
                            RESET_ALL=Style.RESET_ALL))
                    ...
                    print("{WHITE}Egaliter {RESET_ALL}".format(WHITE=Fore.WHITE, LIGHTRED_EX=Fore.LIGHTRED_EX,
                                                               GREEN=Fore.GREEN, RESET_ALL=Style.RESET_ALL))
                    print(
                        "{YELLOW}L'AI a mis un certain nombre independant de COUP: {CYAN} {AICOUP}/{DLCOUP}{YELLOW}({CYAN} {PERCENT}%{YELLOW}) {RESET_ALL}".format(
                            AICOUP=AICOUP, DLCOUP=DLCOUP, CYAN=Fore.CYAN, YELLOW=Fore.YELLOW,
                            PERCENT=(AICOUP / DLCOUP) * 100, LIGHTRED_EX=Fore.LIGHTRED_EX, GREEN=Fore.GREEN,
                    ...
                    if check_victory("O"):
                        ...
                        print(
                            "{LIGHTGREEN_EX}Victoire de l'{RED}IA {RESET_ALL}".format(LIGHTGREEN_EX=Fore.LIGHTGREEN_EX,
                                                                                      RED=Fore.RED, WHITE=Fore.WHITE,
